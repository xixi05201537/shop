...
export function PaymentRequestClient({
  ...
  usePayPalButtons({
    ...
    currency,
    containerId: "payment-request-paypal-buttons",
  ...
  useEffect(() => {
    if (!payable) return;
    const target = paymentRef.current;
    if (!target) return;
  ...
  return (
    <div className="selection-checkout-paypal" id="payment" ref={paymentRef}>
      ...
      {paypalClientId ? (
        <div id="payment-request-paypal-buttons" />
      ) : (
        <div className="selection-status-note is-locked">PayPal checkout is not configured.</div>
      )}
